...
export default function Deploy() {
	...
	const deleteDeploy = async () => {
		...
		try {
			await axios.delete(
				API.deployment + `?namespace=${selectedDeploy?.namespace}&serviceName=${selectedDeploy?.podName}&devName=${selectedDeploy?.developerName}`,
				{
					headers: {
						'auth-token': user?.user?.data?.token,
					},
	...
	useEffect(() => {
		if (location) {
			;(async () => {
				const res = await axios(API.gitlabTagImages + `?projectID=${forTag?.project_id}&registryID=${forTag?.id}`, {
					headers: {
						'auth-token': user?.user?.data?.token,
					},
				})
				...
				<form onSubmit={formik.handleSubmit}>
					...
					<div className="flex items-center space-x-4">
						...
						<Input
							...
							onChange={(e: React.ChangeEvent<HTMLInputElement>) => formik.setFieldValue('imageTag', e.target.value)}
					...
					<div className="flex items-center space-x-4">
						<Input
							...
							onChange={(e: React.ChangeEvent<HTMLInputElement>) => formik.setFieldValue('imagePullSecrets', e.target.value)}
						...
						<Input
							...
							label="Namespace"
							onChange={(e: React.ChangeEvent<HTMLInputElement>) => formik.setFieldValue('namespace', e.target.value)}
					...
					<div>
						...
						>
							<AiOutlinePlusCircle size={22} /> <span>{addons.length > 0 ? 'Add More Support Service' : 'Support Services'}</span>
						</button>
					</div>
					<div className="mt-4">
					...
					<div className="flex justify-center items-center space-x-6 mt-6">
						...
						<button disabled={formik.isSubmitting} className="px-8 py-2 rounded-2xl bg-green text-white disabled:bg-gray-300" type="submit">
